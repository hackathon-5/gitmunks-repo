...
class CommentsController extends AppController
{
    ...
    public function add($trip_id)
    {
        ...
        if ($this->request->is('post')) {
            $data = ['trip_id'=>$trip_id, 'user_id'=>$this->Auth->user('id')] + $this->request->data;
            $comment = $this->Comments->patchEntity($comment, $data);
            $this->Comments->save($comment);
        }
        return $this->redirect('/trips/view/'.$trip_id);
    }
    ...
    public function edit($id = null)
    {
        ...
        $trips = $this->Comments->Trips->find('list', ['limit' => 200]);
        $this->set(compact('comment', 'users', 'trips'));
        $this->set('_serialize', ['comment']);
    }
